...
- addAdjustmentLayer(type, params): brightnessContrast, vibrance, exposure, hueSaturation
- drawShape(shapeType, x0, y0, x1?, y1?, fillColor?, strokeColor?, strokeWidth?, filled?, sides?, points?): Draw ellipse/circle, rect, line, star, polygon, arrow, heart. For circle use shapeType="ellipse" with equal-width bounding box. For star/polygon/heart, x0,y0 is the CENTER and x1,y1 defines the radius (distance from center).
- drawBrushStroke(points: [{x,y}], color?, size?, opacity?, hardness?): Freehand brush stroke through a list of normalized points. Use for organic shapes or scribbles. At least 2 points.
...
export async function runAgent({ prompt }: RunParams): Promise<void> {
  ...
  // First turn: AUTO — let the model decide whether to call tools or reply.
  // Subsequent turns after tool responses: also AUTO.
...
export function commitPreview(): void {
  ...
  // The workspace is gone at this point (we only kept data URLs in the preview).
  // We need to recreate the workspace's mutated state. The simplest approach:
  // re-apply the changes by decoding the `afterDataUrl` and writing it onto
  // the active layer's canvas. This is exactly what a manual "apply filter"
  // does — replace the layer's pixels and push history.
  const s = useEditorStore.getState();
  ...
  // Load the after image into the active layer.
  const img = new Image();
  img.onload = () => {
