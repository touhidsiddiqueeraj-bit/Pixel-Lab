...
export const TOOL_DECLARATIONS: GeminiFunctionDeclaration[] = [
  ...
  },
  {
    ...
    description:
      'Apply a baked-in brightness / contrast / vibrance / exposure / hue-saturation adjustment ' +
      'to the active layer. Despite the name, in v1 this BAKES the adjustment into the active ' +
      'layer pixels (it does NOT create a re-editable non-destructive adjustment layer). ' +
      'Prefer adjustDevelop for the same effect with a clearer name. type is one of: ' +
      '"brightnessContrast" { brightness: -100..100, contrast: -100..100 }, ' +
      '"hueSaturation" { saturation: -100..100 } (hue/lightness not yet supported — falls back to saturation), ' +
  ...
  },
  {
    ...
    description:
      ...
      'Geometric shapes: "star", "polygon", "arrow", "heart". ' +
      'Specialty shapes: "speechBubble" (rounded rect with a tail — use x0,y0,x1,y1 as the bubble bounding box), ' +
      '"spiral" (use x0,y0 as center, x1,y1 defines the max radius via distance from center, turns param controls count). ' +
    ...
    parameters: {
      ...
      properties: {
        shapeType: {
          type: 'string',
          description: 'One of: ellipse (also covers circle when width==height), rect, line, star, polygon, arrow, heart, speechBubble, spiral.',
        },
        ...
        turns: { type: 'number', description: 'For spiral: number of turns (1-20). Default 3.' },
      },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        smooth: { type: 'number', description: 'Path smoothing strength 0-100 (0 = no smoothing, 100 = heavy). Default 0.' },
      },
      required: ['points'],
    },
  },
  {
    ...
    description:
      'Draw a calligraphy stroke — a flat, angle-aware brush that produces thick-and-thin lines like a ' +
      'calligraphy pen. The brush is a filled quad oriented at the calligraphy angle, so the stroke width ' +
      'varies with the angle of motion relative to the pen angle. Use this for hand-lettering, decorative ' +
      'borders, or italic-style strokes. Coordinates are normalized 0-1 with origin at top-left.',
    parameters: {
      type: 'object',
      properties: {
        points: {
        ...
        color: { type: 'string', description: 'CSS color for the stroke. Default "#000000".' },
        size: { type: 'number', description: 'Brush width in pixels (1-100). Default 20.' },
        angle: { type: 'number', description: 'Pen angle in degrees (0-360). 0 = horizontal pen (thick vertical strokes), 45 = classic italic, 90 = vertical pen (thick horizontal strokes). Default 45.' },
        opacity: { type: 'number', description: 'Stroke opacity 0-100. Default 100.' },
      },
      required: ['points'],
    },
  },
  {
    ...
    description:
      'Draw a scatter brush stroke — distributes small filled circles along a path with random offsets, ' +
      'producing a textured/spray-paint effect. Use this for foliage, fur, texture, or decorative scatter. ' +
      'Coordinates are normalized 0-1 with origin at top-left.',
    parameters: {
      type: 'object',
      properties: {
        ...
        color: { type: 'string', description: 'CSS color for the scattered dots. Default "#000000".' },
        count: { type: 'number', description: 'Number of shapes to scatter along the path (1-500). Default 50.' },
        sizeScale: { type: 'number', description: 'Size scale factor for each dot (0.1-5.0). Default 1.0.' },
        opacity: { type: 'number', description: 'Stroke opacity 0-100. Default 80.' },
      },
      required: ['points'],
    },
  },
...
export async function executeTool(
  ...
  switch (toolName) {
    ...
    case 'drawShape': {
      ...
      const drawTo = (target: CanvasRenderingContext2D) => {
        ...
        switch (shapeType) {
          ...
          case 'speechbubble':
          case 'speech-bubble': {
            // Bounding box (x0,y0)-(x1,y1). drawSpeechBubble expects x,y,w,h.
            const bx = Math.min(x0, x1);
            const by = Math.min(y0, y1);
            const bw = Math.max(4, Math.abs(x1 - x0));
            const bh = Math.max(4, Math.abs(y1 - y0));
            drawSpeechBubble(target, bx, by, bw, bh, {
              fillColor, strokeColor, strokeWidth, filled,
            });
            break;
          }
          case 'spiral': {
            const cx = x0;
            const cy = y0;
            const maxRadius = Math.max(4, Math.hypot(x1 - x0, y1 - y0) / 2);
            const turns = clamp(num(args.turns, 3), 1, 20);
            drawSpiral(target, cx, cy, maxRadius, turns, {
              fillColor, strokeColor, strokeWidth, filled,
            });
            break;
          }
    ...
    case 'drawBrushStroke': {
      ...
      const smooth = clamp(num(args.smooth, 0), 0, 100);

      // Convert normalized points to canvas pixels.
      let pts = rawPoints.map((p) => {
      ...
      // Optional path smoothing using the existing smoothPath helper.
      if (smooth > 0 && pts.length >= 3) {
        pts = smoothPath(pts, smooth);
      }
    ...
    // ----- drawCalligraphy -----
    case 'drawCalligraphy': {
      const rawPoints = Array.isArray(args.points) ? args.points : [];
      if (rawPoints.length < 2) {
        return { success: false, message: 'drawCalligraphy requires at least 2 points.' };
      }
      const color = str(args.color, '#000000') || '#000000';
      const size = clamp(num(args.size, 20), 1, 100);
      const angle = clamp(num(args.angle, 45), 0, 360);
      const opacity = clamp(num(args.opacity, 100), 0, 100);
      ...
      const selectionMask = ws.selectionMask;
      const drawTo = (target: CanvasRenderingContext2D) => {
        drawCalligraphyStroke(target, pts, size, angle, color, opacity);
      };
      ...
      try {
        ...
        return {
          success: true,
          message: `Calligraphy stroke (${pts.length} pts, size ${size}, angle ${angle}°, ${color})`,
          thumbnailBase64: generateThumbnail(layer!.canvas, 64),
        };
      } catch (e) {
        return { success: false, message: (e as Error).message };
    ...
    // ----- drawScatterStroke -----
    case 'drawScatterStroke': {
      const rawPoints = Array.isArray(args.points) ? args.points : [];
      if (rawPoints.length < 2) {
        return { success: false, message: 'drawScatterStroke requires at least 2 points.' };
      }
      const color = str(args.color, '#000000') || '#000000';
      const count = Math.round(clamp(num(args.count, 50), 1, 500));
      const sizeScale = clamp(num(args.sizeScale, 1.0), 0.1, 5.0);
      const opacity = clamp(num(args.opacity, 80), 0, 100);
      ...
      const selectionMask = ws.selectionMask;
      const drawTo = (target: CanvasRenderingContext2D) => {
        drawScatterStroke(target, pts, count, sizeScale, color, opacity);
      };
      ...
      try {
        ...
        return {
          success: true,
          message: `Scatter stroke (${pts.length} pts, ${count} dots, scale ${sizeScale}, ${color})`,
          thumbnailBase64: generateThumbnail(layer!.canvas, 64),
        };
      } catch (e) {
        return { success: false, message: (e as Error).message };
...
export function describeToolCall(
  ...
  switch (toolName) {
    ...
    case 'drawCalligraphy': {
      ...
      return `Calligraphy: ${pts} pts, size ${size}, ${angle}°, ${color}`;
    }
    case 'drawScatterStroke': {
      const pts = Array.isArray(args.points) ? args.points.length : 0;
      const color = String(args.color ?? 'black');
      const count = args.count ?? 50;
      return `Scatter: ${pts} pts, ${count} dots, ${color}`;
    }
    case 'addText': {
