...
// NOTE: The previous version was too aggressive at low tolerance values because:
//   1. It only sampled 4 corners (if the subject touched a corner, the whole
//      subject color was treated as "background" and eaten).
//   2. The threshold formula was non-linear — at tolerance=1 it was already
//      quite loose (per-channel distance ~26).
// This version samples MORE edge pixels (every 10px along each edge) and uses
// a more conservative threshold curve so the subject is preserved even at
...
export function autoRemoveBackground(
  ...
  // Sample background color from MORE locations along all 4 edges (not just
  // the 4 corners). This prevents the subject's color from being misdetected
  // as "background" when the subject touches an edge or corner.
  const bgSamples: RGB[] = [];
  const sampleStep = Math.max(1, Math.floor(Math.min(w, h) / 20)); // ~20 samples per edge
  ...
  // Deduplicate samples that are very close in color (reduces false positives
  // from gradient backgrounds).
  const dedupedSamples: RGB[] = [];
  for (const s of bgSamples) {
    let isDup = false;
    for (const d of dedupedSamples) {
      if (Math.abs(s.r - d.r) < 8 && Math.abs(s.g - d.g) < 8 && Math.abs(s.b - d.b) < 8) {
        isDup = true;
        break;
      }
    }
    if (!isDup) dedupedSamples.push(s);
  }

  // Use a more conservative threshold curve.
  // Old: (tol/100) * 67500 → at tol=1, threshold=675 (per-channel ~26) — too loose.
  // New: (tol/100)^2 * 30000 → at tol=1, threshold=3 (nearly exact); at tol=50, threshold=7500.
  // This makes low tolerance values actually conservative.
  const tolFactor = (tolerance / 100) * (tolerance / 100);
  const threshold = tolFactor * 30000;
  ...
  while (stack.length > 0) {
    ...
    // Use a STRICTER check: the pixel must be close to the EDGE pixel that
    // started the flood-fill, not just any background sample. This prevents
    // the flood from "jumping" to a different-colored region of the background
    // and eating into the subject.
    // We achieve this by checking against the nearest edge sample.
    let isBg = false;
