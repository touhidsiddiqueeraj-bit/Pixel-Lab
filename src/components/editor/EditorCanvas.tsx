...
export function EditorCanvas() {
   ...
   * that radius and returns it. Uses a simple Sobel gradient magnitude on the
   * composite canvas. If no strong edge is found within the radius, returns
   * the original point (no snapping).
   *
   * This is what makes the magnetic lasso "magnetic" — it follows edges
   * instead of the exact mouse path.
   */
    ...
    try {
      ...
      for (let y = 1; y < rh - 1; y++) {
        for (let x = 1; x < rw - 1; x++) {
          const idx = (y * rw + x) * 4;
          // Get luminance for each neighbor (use green channel as proxy for speed)
      ...
      // Only snap if we found a meaningful edge (threshold to avoid noise)
      if (bestGrad > 1000) {
        return { x: bestX, y: bestY };
      }
    } catch {
      // getImageData might fail if the region is outside the canvas
    }
    return pt;
  }, [docWidth, docHeight]);
  ...
  const onPointerMove = useCallback((e: React.PointerEvent) => {
    ...
    if (activeTool === 'lasso' || activeTool === 'magnetic-lasso') {
      ...
      if (!last || Math.hypot(pt.x - last.x, pt.y - last.y) > 2) {
        if (activeTool === 'magnetic-lasso') {
          ...
          // a search radius. This uses a simple Sobel gradient magnitude check
          // on the composite canvas to find edges.
          const snapped = snapToEdge(pt, 12);
    ...
    createLassoMask, snapToEdge, docWidth, docHeight, composite, foreground, background,
