...
export function EditorCanvas() {
   ...
   * canvas. If no strong edge is found, returns the original point (no snapping).
   *
  ...
  const snapToEdge = useCallback((pt: Point, radius: number): Point => {
    ...
    try {
      ...
      let bestY = pt.y;
      let bestGrad = 0;
      // Sobel edge detection on luminance (use green channel as luminance proxy
      // — it's the dominant channel for human perception and avoids the extra
      // math of a full RGB-to-luma conversion).
      for (let y = 1; y < rh - 1; y++) {
        for (let x = 1; x < rw - 1; x++) {
          const tl = data[((y - 1) * rw + (x - 1)) * 4 + 1];
          const tc = data[((y - 1) * rw + x) * 4 + 1];
          const tr = data[((y - 1) * rw + (x + 1)) * 4 + 1];
          const ml = data[(y * rw + (x - 1)) * 4 + 1];
          const mr = data[(y * rw + (x + 1)) * 4 + 1];
          const bl = data[((y + 1) * rw + (x - 1)) * 4 + 1];
          const bc = data[((y + 1) * rw + x) * 4 + 1];
          const br = data[((y + 1) * rw + (x + 1)) * 4 + 1];
      ...
      // Only snap if we found a meaningful edge. The threshold of 1000 means
      // a gradient magnitude of ~32 per channel — enough to avoid noise but
      // catch real edges.
      if (bestGrad > 1000) {
        return { x: bestX, y: bestY };
      }
    } catch {
      // getImageData might fail if the canvas isn't ready
    }
    return pt;
  }, [docWidth, docHeight]);
  ...
  const onPointerMove = useCallback((e: React.PointerEvent) => {
    ...
    if (activeTool === 'lasso' || activeTool === 'magnetic-lasso') {
      ...
      // Live preview as selection (this also triggers the overlay redraw via
      // the animation loop). But for immediate visual feedback (no 15fps lag),
      // we also update the cursor position and call drawOverlay directly.
      setCursorPos(pt);
      const result = createLassoMask(lassoPointsRef.current);
      if (result) setSelection(result.mask, result.bounds);
      // Direct overlay redraw for immediate visual feedback of the lasso path
      drawOverlay();
      return;
    }
