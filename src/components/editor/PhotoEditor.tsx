...
export function PhotoEditor() {
  ...
  return (
    ...
    >
      ...
      {/* AI Agent floating bubble (bottom-left, all viewports).
          Shows a pulse when no API key is set (to draw attention on first use),
          a spinner when a run is in progress, and a badge dot when a preview
          is awaiting Accept/Reject. Click opens the agent panel in a Sheet. */}
      <Button
        onClick={() => setAgentSheetOpen(true)}
        className={cn(
          'fixed bottom-4 left-4 z-50 h-12 w-12 rounded-full shadow-lg touch-target',
          'bg-gradient-to-br from-sky-500 to-purple-600 hover:from-sky-400 hover:to-purple-500',
          'text-white border border-white/20',
          // Pulse to draw attention when no API key is set yet
      ...
      >
        ...
        {/* Badge dot when there's an error */}
        {agentStatus === 'error' && (
          <span className="absolute -top-0.5 -right-0.5 h-3.5 w-3.5 rounded-full bg-red-500 border-2 border-white" />
        )}
      </Button>

      {/* AI Agent panel — opened as a Sheet from the floating bubble.
          On desktop it's a right-side sheet (380px wide); on mobile it's
          full-width. The Sheet keeps the agent accessible without taking up
          permanent screen real estate. */}
      <Sheet open={agentSheetOpen} onOpenChange={setAgentSheetOpen}>
        <SheetContent
          side="right"
          className="editor-surface editor-text border-l editor-border p-0 w-full sm:w-[380px] flex flex-col"
        >
