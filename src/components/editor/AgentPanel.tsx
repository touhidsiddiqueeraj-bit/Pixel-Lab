...
export function AgentPanel() {
  ...
  }, [prompt, apiKey, activeLayerId, layers.length, status]);
      ...
      <div className="border-t editor-border p-2 shrink-0 space-y-2">
        ...
        <div className="flex items-end gap-1">
          <textarea
            ...
            disabled={isRunning || isAwaitingAccept || !apiKey}
            placeholder={
              !apiKey
                ? 'Enter your API key above first…'
                : isAwaitingAccept
                  ? 'Accept or reject the preview above first…'
                  : isRunning
                    ? 'Agent is running…'
          ...
          {isRunning ? (
            ...
            <Button
              size="sm"
              onClick={handleSend}
              disabled={!prompt.trim() || !apiKey || isAwaitingAccept}
