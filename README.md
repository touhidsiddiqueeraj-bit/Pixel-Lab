...
<p align="center">
  A professional, web-based image editor built with Next.js, TypeScript, and Canvas API. 100% client-side — no cloud dependency. Features 40 tools, layers, masks, Lightroom-style develop panel, vectorization, LUT color grading, content-aware fill, an AI editing agent powered by Gemini, and a fully responsive mobile-friendly interface.
</p>
...
- **14 agent tools** covering filters, develop adjustments, selection, drawing, text, and bucket fill
- **Tool-calling loop** against Google's Gemini API (`gemini-2.5-flash-lite` / `flash` / `pro`)
- **Offscreen preview** — every edit runs on a cloned canvas; nothing touches the live canvas or undo stack until you click Accept
...
- **API key security**: key lives only in browser JS memory (never persisted to localStorage); calls go directly to `generativelanguage.googleapis.com`, never to any backend of ours. See [SECURITY_NOTES.md](SECURITY_NOTES.md) for honest disclosure.
- **Model picker** with preference persisted (Flash-Lite default)
- Try prompts like: *"make it grayscale"*, *"brighten the sky and add a vignette"*, *"draw a red circle in the center"*, *"fill the background blue"*, *"write Hello World in green at the top-left"*
...
3. Paste your key in the API key input (it stays in browser memory only — never persisted)
...
**How it works:**
- The agent snapshots your canvas into an **offscreen workspace** before running
- Each tool call mutates the workspace, not the live canvas — you see progress chips in the chat thread
- When the model finishes, you get a **before/after preview** with Accept / Reject
- **Accept** commits to the layer + pushes a history entry (Ctrl+Z undoes it normally)
- **Reject** discards the preview without touching the undo stack
- **Stop** cancels an in-flight run; **MAX_TOOL_CALLS = 8** prevents infinite loops
...
- **Icons**: Lucide React
- **Toasts**: Sonner
- **No cloud dependency**: 100% client-side processing (the only external API call is the user's own Gemini key, sent directly to Google)
...
- [Security Notes](SECURITY_NOTES.md) — Honest disclosure of AI agent API key handling
