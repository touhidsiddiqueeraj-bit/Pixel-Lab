...
40 tools · Layers & masks · Lightroom-style develop · Vectorization · LUT color grading · Content-aware fill · **AI editing agent powered by Gemini** · Fully responsive
...
- [Usage](#usage)
  ...
  - [Pro Tips](#pro-tips)
  - [AI Editing Agent](#ai-editing-agent)
...
Pixel Lab is a browser-based image editor built with Next.js 16, TypeScript, and the HTML5 Canvas API. The entire editing pipeline runs in the browser — there is no server-side image processing. It features 40 tools, layers with non-destructive masks, a Lightroom-style develop panel, raster-to-SVG vectorization, LUT color grading, content-aware fill, and a **Gemini-powered AI editing agent** that translates natural-language prompts into real editor operations.
...
A Copilot-Chat-style panel that translates natural-language prompts into real editor operations, with a before/after preview you accept or reject.

<div align="center">

| | |
|---|---|
| **14 agent tools** | Filters, develop adjustments, selection, drawing, text, bucket fill |
| **Tool-calling loop** | Against Google's Gemini API (`gemini-2.5-flash-lite` / `flash` / `pro`) |
| **Offscreen preview** | Every edit runs on a cloned canvas — live canvas & undo stack never touched until Accept |
| **Accept / Reject** | Accept commits to history (Ctrl+Z undoes normally); Reject discards without touching history |
...
See [SECURITY_NOTES.md](SECURITY_NOTES.md) for honest disclosure of the API key handling, and [ARCHITECTURE.md → AI Editing Agent](ARCHITECTURE.md#ai-editing-agent-gemini-powered) for the full architecture.
...
> 📸 **AI Agent screenshots**: Verification screenshots for the AI editing agent (drawing tools, accept/reject, multi-step prompts, etc.) are in the development artifacts at `/home/z/my-project/download/agent-screenshots/` — not bundled with the repo to keep it lean.
...
3. When the model finishes, you get a **before/after preview** with Accept / Reject
4. **Accept** commits to the layer + pushes a history entry (Ctrl+Z undoes it normally)
5. **Reject** discards the preview without touching the undo stack
6. **Stop** cancels an in-flight run; **MAX_TOOL_CALLS = 8** prevents infinite loops

**Colors:** Any CSS color works — hex (`#ff0000`), named (`red`, `blue`, `yellow`), `rgb()`, or `hsl()`.

**Security:** Your API key is sent only to `generativelanguage.googleapis.com` — never to any backend of ours, never persisted to localStorage. See [SECURITY_NOTES.md](SECURITY_NOTES.md) for the full honest disclosure.
...
| **AI Agent** | Google Gemini API (`generateContent` with `functionDeclarations`) — called directly from the browser, no backend proxy |
| **Icons** | Lucide React |
| **Toasts** | Sonner |
| **Database** | Prisma (optional — for app-level features like recent files, not for image data) |
| **Package Manager** | Bun (recommended) or npm |

**No cloud dependency:** 100% client-side processing. The only external API call is the user's own Gemini key, sent directly to Google by the AI agent.
...
│   │   └── agent/              # AI editing agent (Gemini-powered)
│   │       ├── agent-store.ts  # Zustand slice — in-memory API key, chat thread, pending preview
│   │       ├── gemini-client.ts# Thin wrapper around Gemini generateContent with functionDeclarations
│   │       ├── tools.ts        # 14-tool schema + executor wrapping existing editor functions
│   │       └── agent-runner.ts # Orchestration loop — offscreen workspace, MAX_TOOL_CALLS, commit/reject
│   └── components/
...
├── EXTENSIONS.md               # How to create extensions, connect AI agents via MCP
├── CONTRIBUTING.md             # Contribution guide
├── SECURITY_NOTES.md           # Honest disclosure of AI agent API key handling
├── LICENSE                     # MIT License
└── README.md                   # This file
```
...
| Doc | What it covers |
|---|---|
| [**Architecture**](ARCHITECTURE.md) | System design, data flow diagrams, canvas rendering engine, Luna AI agent loop, performance system, extension points |
...
| [**Security Notes**](SECURITY_NOTES.md) | Honest disclosure of Luna API key handling — what's safe, what isn't, what you can do |
...
- AI Editing Agent with 14 tools (filters, develop, selection, drawing, text, bucket fill)
...
- **Per-step accept/reject** — currently the agent produces a single combined preview per turn; a future config option could allow per-step review
...
- **More AI agent tools** — pen tool paths, gradient fills, layer effects, perspective transform
...
- How to add a new tool, filter, or AI agent tool
- The PR process and review criteria
- Test scripts for the AI agent
...
- AI agent powered by [Google Gemini](https://ai.google.dev/)
