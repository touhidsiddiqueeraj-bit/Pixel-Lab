...
- **🆕 Vision self-evaluation**: After the tool-calling loop finishes, a
  vision model reviews BEFORE vs AFTER images and scores the edit 1–10.
  Scores below 7 trigger an automatic retry with feedback (up to 2 retries),
  so bad edits are caught before the user sees them. The self-eval uses a
  stronger vision model than the user's tool-calling model (Flash-Lite →
  Flash for vision) for accurate quality assessment.
- **🆕 Preference memory**: Every Accept/Reject is recorded to
  `localStorage` (`pixel-lab-agent-preferences` key, max 50 entries). On
  subsequent runs, `buildPreferenceSummary()` derives a textual profile
  (accept rate, most-accepted/most-rejected tool types, recent examples,
  self-eval agreement rate) and appends it to the system prompt — so Luna
  adapts to the user's taste over time. The brain icon in the Luna header
  opens a panel showing these stats; "Clear memory" wipes the history.
- **Access**: Click the floating ✨ button at the bottom-right, or use the
  "Luna" tab in the right panel.

See [SECURITY_NOTES.md](SECURITY_NOTES.md) for API key handling and
preference memory storage details. See
[ARCHITECTURE.md → AI Editing Agent](ARCHITECTURE.md#ai-editing-agent-gemini-powered)
for the full architecture including the self-eval + retry loop and
preference memory data model.
...
- **Per-step accept/reject for Luna**: Currently Luna produces a single
  combined preview per turn (with self-eval retries happening
  transparently). A future config option could allow per-step review.
- **Tunable self-eval threshold**: Currently fixed at 7/10. A setting could
  let advanced users make Luna stricter or more permissive.
- **Per-tool preference breakdown UI**: The preference memory already tracks
  which tools correlate with accept/reject; surfacing this in a dedicated
  panel would help users understand their own patterns.
