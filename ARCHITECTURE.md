...
├── gemini-client.ts # Thin wrapper around Gemini generateContent REST endpoint
├── tools.ts         # 14-tool schema + executor (wraps existing editor functions)
...
The model preference (Flash-Lite / Flash / Pro) **is** persisted to `localStorage` — it is a non-secret UI preference, identical in sensitivity to a theme choice. See `SECURITY_NOTES.md` for the full honest disclosure, including the limitations of any client-side API key flow (XSS, malicious extensions, device access).

### Tool-Calling Loop

```
┌─────────────────────────────────────────────────────────────┐
│  User types "draw a red circle in the center"               │
└──────────────────────────┬──────────────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────────────┐
│  agent-runner.ts: snapshotWorkspace()                       │
│  Deep-clone all layers + selection mask into offscreen      │
│  workspace (NEVER touch live editor-store during the loop)  │
└──────────────────────────┬──────────────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────────────┐
│  gemini-client.ts: generateContent()                        │
│  POST to generativelanguage.googleapis.com                  │
│  with functionDeclarations + canvas as 1024px JPEG inline   │
└──────────────────────────┬──────────────────────────────────┘
                           ▼
                  ┌────────────────┐
                  │ Model returns? │
                  └────┬───────┬───┘
                       │       │
              text-only│       │ functionCall(s)
                       │       │
                       ▼       ▼
     ┌─────────────────┐  ┌────────────────────────────────┐
     │ Build preview   │  │ For each call:                 │
     │ (before/after)  │  │  - Execute against WORKSPACE   │
     │ → awaiting-     │  │  - Emit progress chip          │
     │   accept        │  │  - Send functionResponse back  │
     └─────────────────┘  │  - Repeat (goto generateContent)│
                          └────────────────────────────────┘
                                       │
                                       ▼
                          Hard stop at MAX_TOOL_CALLS = 8
                          (surfaces error instead of looping)
```
...
- The 14 available tools and their parameter ranges
