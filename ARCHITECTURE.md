...
flowchart TD
    ...
    BEFORE --> IMG[Downscale composite to 1024px JPEG<br/>as Gemini inline image part]
    IMG --> GEN
    ...
    PREV["Build before/after preview<br/>compositeWorkspace(ws) → afterDataUrl<br/>Set status: awaiting-accept"]
    PREV --> UI

    UI[AgentPanel UI shows<br/>before/after preview<br/>+ Accept / Reject buttons]
    UI -- User clicks Accept --> COMMIT
    UI -- User clicks Reject --> REJECT
    UI -- User clicks Stop --> END_CANCEL

    COMMIT["commitPreview()<br/>Load afterDataUrl into active layer<br/>pushHistory() — same structure as manual edit<br/>Ctrl+Z undoes it normally"]
    COMMIT --> DONE[Status: done]

    REJECT["rejectPreview()<br/>Discard preview<br/>Undo stack UNTOUCHED<br/>(workspace was offscreen all along)"]
