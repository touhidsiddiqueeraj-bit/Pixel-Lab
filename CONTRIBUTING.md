...
| `agent/tools.ts` | 14-tool schema + executor for the AI agent. Read this before adding a new agent tool. See [ARCHITECTURE.md → AI Editing Agent](ARCHITECTURE.md#ai-editing-agent-gemini-powered) for the full pattern. |
| `agent/agent-runner.ts` | Orchestration loop. Captures an offscreen workspace snapshot, runs the tool-call loop, builds the before/after preview. |
| `AgentPanel.tsx` | UI for the agent — chat thread, model picker, API key input, Accept/Reject preview. |
